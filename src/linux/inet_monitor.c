...
static const char *tcp_states_map[] = {[TCP_ESTABLISHED] = "ESTABLISHED",
...
int main(int argc, char *argv[]) {
    ...
    // The requests can (will in most cases) come as multiple netlink messages.
    // I need to receive all of them. Assumes no packet loss, so if the last
    // packet (the packet with NLMSG_DONE) is lost, the application will hang.
    while (1) {
